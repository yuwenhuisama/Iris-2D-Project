...
namespace Iris2D{
	...
	bool IrisBitmap::Blt(int x, int y, const IIrisBitmap *srcIrisBitmap, const IIrisRect *srcIrisRect, int opacity, IR_PARAM_RESULT_CT){
		this->needRefreshTexture = true;
		TYPE_CHANGE_TO_VARIABLE(isrcIrisRect, srcIrisRect, const IrisRect*)
		TYPE_CHANGE_TO_VARIABLE(isrcIrisBitmap, srcIrisBitmap, const IrisBitmap*)
		Rect rc((int)isrcIrisRect->x, (int)isrcIrisRect->y, (int)isrcIrisRect->width, (int)isrcIrisRect->height);
		bool r = IrisBitmapBitBlt(x, y, this->bitmap, isrcIrisBitmap->bitmap, &rc, opacity);
		if (r)
			IR_PARAM_SET_RESULT(IR_RSUCCESS, L"")
		else
	...
	bool IrisBitmap::StretchBlt(const IIrisRect *destIrisRect, const IIrisBitmap *srcIrisBitmap, const IIrisRect *srcIrisRect, int opacity, IR_PARAM_RESULT_CT){
		this->needRefreshTexture = true;
		TYPE_CHANGE_TO_VARIABLE(isrcIrisRect, srcIrisRect, const IrisRect*)
		TYPE_CHANGE_TO_VARIABLE(idestIrisRect, destIrisRect, const IrisRect*)
		TYPE_CHANGE_TO_VARIABLE(isrcIrisBitmap, srcIrisBitmap, const IrisBitmap*);
		Rect rc((int)isrcIrisRect->x, (int)isrcIrisRect->y, (int)isrcIrisRect->width, (int)isrcIrisRect->height);
		Rect drc((int)idestIrisRect->x, (int)idestIrisRect->y, (int)idestIrisRect->width, (int)idestIrisRect->height);
		bool r = IrisBitmapStretchBlt(&drc, this->bitmap, &rc, isrcIrisBitmap->bitmap, opacity);
		if (r)
			IR_PARAM_SET_RESULT(IR_RSUCCESS, L"")
		else
	...
	void IrisBitmap::FillRect(int x, int y, int width, int height, const IIrisColor *color, IR_PARAM_RESULT_CT){
		this->needRefreshTexture = true;
		...
		SolidBrush *tpBrush = new SolidBrush(Color(icolor->alpha, icolor->red, icolor->green, icolor->blue));
	...
	void IrisBitmap::FillRect(const IIrisRect *rect, const IIrisColor *color, IR_PARAM_RESULT_CT){
		TYPE_CHANGE_TO_VARIABLE(irect, rect, const IrisRect*)
		this->FillRect((int)irect->x, (int)irect->y, (int)irect->width, (int)irect->height, color, IR_PARAM);
	}
	...
	void IrisBitmap::ClearRect(const IIrisRect* rect, IR_PARAM_RESULT_CT){
		TYPE_CHANGE_TO_VARIABLE(irect, rect, const IrisRect*)
		this->ClearRect((int)irect->x, (int)irect->y, (int)irect->width, (int)irect->height, IR_PARAM);
	}

	IIrisColor* IrisBitmap::GetPixel(int x, int y, IR_PARAM_RESULT_CT){
		Color color;
	...
	void IrisBitmap::SetPixel(int x, int y, const IIrisColor *color, IR_PARAM_RESULT_CT){
		this->needRefreshTexture = true;
		TYPE_CHANGE_TO_VARIABLE(icolor, color, const IrisColor*)
		Color *gdiColor = new Color(icolor->alpha, icolor->red, icolor->green, icolor->blue);
	...
	void IrisBitmap::GradientFillRect(int x, int y, int width, int height, const IIrisColor *color1, const IIrisColor *color2, bool vertical, IR_PARAM_RESULT_CT){
	}

	void IrisBitmap::GradientFillRect(const IIrisRect *rect, const IIrisColor *color1, const IIrisColor *color2, bool vertical, IR_PARAM_RESULT_CT){
	}
	...
	int IrisBitmap::TextSize(const IIrisFont* fontIris, wstring str, IR_PARAM_RESULT_CT){
		int size = IrisFont::defaultSize;
		IrisColor *color = static_cast<IrisColor*>(IrisFont::defaultColor);
		wstring name;
		bool bold, shadow, italic;

		TYPE_CHANGE_TO_VARIABLE(ifontIris, fontIris, const IrisFont*)
		...
		else
		{
			name = ifontIris->name;
			bold = ifontIris->bold;
			shadow = ifontIris->shadow;
			italic = ifontIris->italic;
		}
	...
	void IrisBitmap::IrisDrawText(int x, int y, int width, int height, wstring str, int align, IR_PARAM_RESULT_CT){
		...
		IrisFont* fontIris = static_cast<IrisFont*>(this->font);
		...
		if (fontIris == NULL){
			...
			shadow = IrisFont::defaultShadow;
			italic = IrisFont::defaultItalic;
			color = static_cast<IrisColor*>(IrisFont::defaultColor);
		}
		else
		{
			size = fontIris->size;
			...
			color = static_cast<IrisColor*>(fontIris->color);
		}
	...
	void IrisBitmap::IrisDrawText(const IIrisRect *rect, const wstring str, const int align, IR_PARAM_RESULT_CT){
		TYPE_CHANGE_TO_VARIABLE(irect, rect, const IrisRect*)
		IrisDrawText((int)irect->x, (int)irect->y, (int)irect->width, (int)irect->height, str, align, IR_PARAM);
	}

	void IrisBitmap::Draw(const IIrisRect* Rect, IR_PARAM_RESULT_CT){
		bool flag = false;
		...
		TYPE_CHANGE_TO_VARIABLE(iRect, Rect, const IrisRect*)
		if (this->limitRect->x != iRect->x || this->limitRect->y != iRect->y
			|| this->limitRect->width != iRect->width || this->limitRect->height != iRect->height){
			//delete this->limitRect;
		...
		if (flag)
			if (!this->MakeTexture(static_cast<const IrisRect*>(Rect))){
