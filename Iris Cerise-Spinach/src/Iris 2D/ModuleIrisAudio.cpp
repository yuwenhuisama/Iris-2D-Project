...
namespace Iris2D{
	...
	bool ModuleIrisAudio::bgmIsFading = false;
	bool ModuleIrisAudio::bgsIsFading = false;
	bool ModuleIrisAudio::seIsFading = false;
...
#define FALSE 0
	...
	bool ModuleIrisAudio::BgmFade(int fadeTime){
		DWORD threadID;
		if (!bgmIsFading)
			::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FadeBgmThreadProc, (void*)fadeTime, 0, &threadID);
		return true;
	}
	...
	bool ModuleIrisAudio::BgsFade(int fadeTime){
		DWORD threadID;
		if (!bgsIsFading)
			::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FadeBgsThreadProc, (void*)fadeTime, 0, &threadID);
		return true;
	}

	bool ModuleIrisAudio::SeFade(int fadeTime){
		DWORD threadID;
		if (!seIsFading)
			::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)FadeSeThreadProc, (void*)fadeTime, 0, &threadID);
		return true;
	}
	...
	void ModuleIrisAudio::FadeBgsThreadProc(int duration){
		bgsIsFading = true;
		int time = duration;
		while (time >= 0){
			float v;
			FMOD_Channel_SetPaused(bgsChannel, true);
			FMOD_Channel_GetVolume(bgsChannel, &v);
			FMOD_Channel_SetVolume(bgsChannel, v - v / (duration / 1000));
			time -= 1000;
	...
	void ModuleIrisAudio::FadeSeThreadProc(int duration){
		seIsFading = true;
		int time = duration;
		while (time >= 0){
			float v;
			FMOD_Channel_SetPaused(seChannel, true);
			FMOD_Channel_GetVolume(seChannel, &v);
			FMOD_Channel_SetVolume(seChannel, v - v / (duration / 1000));
			time -= 1000;
	...
	void ModuleIrisAudio::FadeBgmThreadProc(int duration){
		...
		while (time >= 0){
			...
			bgmChannel->getVolume(&v);
			bgmChannel->setVolume(v - v / (duration / 1000));
			time -= 1000;
	...
	void ModuleIrisAudio::FadeBgsThreadProc(int duration){
		...
		while (time >= 0){
			float v;
			bgsChannel->setPaused(true);
			bgsChannel->getVolume(&v);
			bgsChannel->setVolume(v - v / (duration / 1000));
			time -= 1000;
	...
	void ModuleIrisAudio::FadeSeThreadProc(int duration){
		...
		while (time >= 0){
			float v;
			seChannel->setPaused(true);
			seChannel->getVolume(&v);
			seChannel->setVolume(v - v / (duration / 1000));
			time -= 1000;
