...
namespace Iris2D{
	...
	bool IrisShader::DoSpriteShade(list<IIrisSprite*>& sprites){
		...
		for (it = sprites.begin(); it != sprites.end(); it++){
			...
			static_cast<IrisBitmap*>(sp->bitmap)->Draw(sp->srcRect);
			//-------------Material && Texture Setting-------------
			IDirect3DTexture9* text = static_cast<IrisBitmap*>(sp->bitmap)->GetTexture();
			...
			//------------BlendType-------------
			//pSurface->Text
			spriteEffect->SetTexture(spriteBackgroundHandle, (static_cast<IrisViewport*>(sp->viewport))->renderTexture);
			...
			for (UINT i = 0; i < numPasses; i++)
			{
				...
				Device->SetStreamSource(0, static_cast<IrisBitmap*>(sp->bitmap)->GetVb(), 0, sizeof(Iris2DVertex));
	...
	bool IrisShader::DoViewportShade(list<IIrisViewport*>& viewports){
		...
		for (it = viewports.begin(); it != viewports.end(); it++){
			vp = static_cast<IrisViewport*>(*it);
