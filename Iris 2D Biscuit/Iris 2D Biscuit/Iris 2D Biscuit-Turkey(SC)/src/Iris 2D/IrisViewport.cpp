...
namespace Iris2D{
	...
	bool IrisViewport::SpriteSortFunction(IrisSprite* sp1, IrisSprite* sp2){ return sp1->GetZ() < sp2->GetZ(); }

	void IrisViewport::BeginBatchSpriteProcess(IrisViewport* vp) { vp == NULL ? ModuleIrisGraphics::GetGViewport()->isBatching = true : vp->isBatching = true; }
	void IrisViewport::EndBatchSpriteProcess(IrisViewport* vp){
		IrisViewport* tvp = vp == NULL ? ModuleIrisGraphics::GetGViewport() : vp;
		tvp->isBatching = false;
		tvp->sortSprites();
	}
	...
	IrisViewport::IrisViewport(const IrisRect *rect, IR_PARAM_RESULT_CT) : IrisViewport(rect->x, rect->y, rect->width, rect->height, IR_PARAM)
	{
	}
	...
	void IrisViewport::SetInitData(const IrisRect *rect, IR_PARAM_RESULT_CT){
		SetInitData(rect->x, rect->y, rect->height, rect->width, IR_PARAM);
	}
