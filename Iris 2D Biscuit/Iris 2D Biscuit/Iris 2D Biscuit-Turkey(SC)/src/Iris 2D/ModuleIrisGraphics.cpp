...
namespace Iris2D{
	...
	bool ModuleIrisGraphics::ViewportSortFunc(IrisViewport*& vp1, IrisViewport*& vp2){
	...
	bool ModuleIrisGraphics::UpdateSpritesAndViewports(IrisShader*& shader, D3DXMATRIX& proj){
		...
		//D3DXSaveTextureToFile(L"3123.bmp", D3DXIFF_BMP, ModuleIrisGraphics::ExchangeTexture, 0);
	...
	void ModuleIrisGraphics::Snap2Bitmap(IrisBitmap *bitmap, IR_PARAM_RESULT_CT){
		bool flag = true;
		IDirect3DSurface9* EXSurface;
		IDirect3DSurface9* ExchangeSurface;
		if(FAILED(ExchangeTexture->GetSurfaceLevel(0, &EXSurface))) flag = false;
