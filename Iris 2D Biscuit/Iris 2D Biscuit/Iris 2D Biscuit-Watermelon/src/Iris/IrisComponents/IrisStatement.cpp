...
IrisStatementResult IrisStatement::ExcuteForStatement(IrisContextEnvironment* pContexEnvironment) {
	...
	// Array For
	if (!pIter2) {
		if (ivSource.GetObjectPointer()->GetClass()->GetClassName() == "Array") {
			...
			for (auto value : vcSource) {
				if (value.GetObjectPointer()->m_bLiteralObject) {
					value.GetObjectPointer()->m_bLiteralObjectAssigned = true;
				}
				ivIter.SetObject(value.GetObjectPointer());
	...
	// Hash For
		...
		if (ivSource.GetObjectPointer()->GetClass()->GetClassName() == "Hash") {
			...
			for (auto value : vcSource) {
				...
				if (((IrisValue&) value.first).GetObjectPointer()->m_bLiteralObject) {
					((IrisValue&)value.first).GetObjectPointer()->m_bLiteralObjectAssigned = true;
				}
				if (value.second.GetObjectPointer()->m_bLiteralObject) {
					value.second.GetObjectPointer()->m_bLiteralObjectAssigned = true;
				}

				ivIter1.SetObject(((IrisValue&)value.first).GetObjectPointer());
				ivIter2.SetObject(value.second.GetObjectPointer());
				srResult = m_uType.m_pForStatement->m_pBlock->Execute(pContexEnvironment);
...
IrisStatementResult IrisStatement::ExcuteSwitchStatement(IrisContextEnvironment* pContexEnvironment) {
	...
	for (auto when : m_uType.m_pSwitchStatement->m_pSwitchBlock->m_pWhenList->m_lsList) {
		for (auto expr : when->m_pExpressions->m_lsList) {
			if (expr->Excute(pContexEnvironment).GetObjectPointer()->CallInstanceFunction("==", pContexEnvironment, &ivParam) == IrisInterpreter::CurInstance()->True()) {
...
IrisStatementResult IrisStatement::ExcuteClassStatement(IrisContextEnvironment* pContexEnvironment) {
	...
	if (!m_uType.m_pClassStatement->m_pSuperClassName) {
		...
		if (ivSuperClass.GetObjectPointer()->GetClass()->GetClassName() != "Class") {
			return srResult;
		}
		pSuperClass = ((IrisClassBaseTag*)ivSuperClass.GetObjectPointer()->m_pNativeObject)->GetClass();
	}
