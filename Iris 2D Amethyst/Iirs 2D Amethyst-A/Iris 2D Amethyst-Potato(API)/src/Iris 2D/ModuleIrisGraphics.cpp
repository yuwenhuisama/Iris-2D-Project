...
bool ModuleIrisGraphics::compareViewprotWithZ(IIrisViewport *v1, IIrisViewport *v2){
...
void ModuleIrisGraphics::Init(){
	gViewport = (IIrisViewport*)(new IrisViewport(0, 0, (float)ModuleIrisGraphics::width, (float)ModuleIrisGraphics::height));
	MakeExchangeTexture();
	MakeExchangeVertex();
}
...
void ModuleIrisGraphics::disposeAllGraphicsSource(){
	...
	list<IIrisViewport*>::iterator it;
	for(it = viewports.begin(); it != viewports.end(); it++){
		((IrisViewport*)(*it))->AutoDispose();
	}

	list<IIrisSprite*> sprites = ((IrisViewport*)ModuleIrisGraphics::gViewport)->GetSprites();
	list<IIrisSprite*>::iterator its = sprites.begin();
	for(its = sprites.begin(); its != sprites.end(); its++){
		((IrisSprite*)(*its))->AutoDispose();
	}
}
