...
bool ModuleIrisInput::CursorInRect(IIrisRect* rect){
	POINT  pt;
	::GetCursorPos(&pt);
	::ScreenToClient(ModuleIrisGraphics::hwnd, &pt);

	if (pt.x >= ((IrisRect*)rect)->x && pt.x <= ((IrisRect*)rect)->x + ((IrisRect*)rect)->width && pt.y >= ((IrisRect*)rect)->y && pt.y <= ((IrisRect*)rect)->y + ((IrisRect*)rect)->height)
		return true;
	return false;
}
