...
namespace Iris2D
{
	...
	*/
	...
	private:
		...
		RectDX* m_pSrcRect = nullptr;
		ToneDX* m_pTone = nullptr;

		bool m_bSrcRectDirtyFlag = false;
		bool m_bToneDirtyFlag = false;

	public:
		static ViewportDX* sm_pGlobalViewport;
	...
	public:
		...
		* @see Create(float fX, float fY, unsigned int nWidth, unsigned int nHeight, IR_PARAM_RESULT);
		*/
		static ViewportDX* Create(const RectDX* pRect, IR_PARAM_RESULT);
		...
		*/
		...
		static void ManagedRelease(ViewportDX* pViewport);
		static void InnerRelease(ViewportDX*& pViewport);

		static bool InitGlobalViewport(unsigned int nWindowWidth, unsigned int nWindowHeight);
		static void ReleaseGlobalViewport();
		static ViewportDX* GetGlobalViewport();
	...
	public:
		...
		*/
		void SetSrcRect(RectDX*& pSrcRect);
		/**
		* \~english
