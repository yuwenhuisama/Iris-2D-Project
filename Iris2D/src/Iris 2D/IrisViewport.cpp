...
IrisViewport::IrisViewport(float x, float y, float width, float height){
	...
	//this->backSprite = new IrisSprite(this->Device, this);
	//this->backSprite->SetBitmap(new IrisBitmap(this->Device, this->GetWidth(), this->GetHeight()));
	//this->backSprite->SetTone(new IrisTone(*this->tone));
	//IrisRect rect(0, 0, this->GetWidth(), this->GetHeight());
	//IrisColor color(0, 0, 0, 0);
	//this->backSprite->GetBitmap()->FillRect(&rect, &color);

	//this->deleteSprite(this->backSprite);
}

IrisViewport::IrisViewport(IrisRect *rect){
	this->rect = rect;
	this->Device = this->Device = ModuleIrisGraphics::Device;
