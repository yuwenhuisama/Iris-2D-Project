...
#include "DirectX/Iris2D/IrisShaders/IrisVertexShaderBase.h"
#include "DirectX/Iris2D/IrisShaders/IrisPixelShaderBase.h"
#include "DirectX/Iris2DUtil/IrisTexture.h"

namespace Iris2D
{
	IrisD3DResourceManager * IrisD3DResourceManager::Instance()
	{
		static auto pInstance = IrisD3DResourceManager();
		return &pInstance;
	}

	bool IrisD3DResourceManager::Initialize(HWND hWindow)
	{
		RECT rcDimensions;
	...
	bool IrisD3DResourceManager::Release()
	{
	...
	ID3D10Device1 * IrisD3DResourceManager::GetD3D10Device()
	{
		return m_pD3D10Device;
	}

	ID3D11DeviceContext * IrisD3DResourceManager::GetD3DDeviceContext()
	{
		return m_pD3D11Context;
	}

	IDXGISwapChain * IrisD3DResourceManager::GetSwapChain()
	{
		return m_pSwapChain;
	}

	ID3D11RenderTargetView * IrisD3DResourceManager::GetRenderTargetView()
	{
		return m_pBackBufferTarget;
	}

	void IrisD3DResourceManager::SetVertexShader(IrisVertexShaderBase * pVertexShader)
	{
	...
	void IrisD3DResourceManager::SetPixelShader(IrisPixelShaderBase * pPixelShader)
	{
		//m_pD3D11Context->PSSetShader(pPixelShader->GetPixelShader(), nullptr, 0);
		pPixelShader->SetToContext();
	}

	void IrisD3DResourceManager::SetCurrentVertexBufferInfo(unsigned int nStride, unsigned int nOffset, ID3D11Buffer * pVertexBuffer)
	{
	...
	void IrisD3DResourceManager::SetCurrentTexture(IrisTexture * pTexture)
	{
		m_pCurrentTexture = pTexture;
	}

	void IrisD3DResourceManager::AdjustViewMatrix(unsigned int nWindowWidth, unsigned int nWindowHeight)
	{
	...
	const DirectX::XMMATRIX& IrisD3DResourceManager::GetViewMatrix()
	{
		return m_mxViewProjMatrix;
	}

	bool IrisD3DResourceManager::InitializeDirectX11(IDXGIAdapter1* pAdapter, int nWidth, int nHeight, HWND hWindow)
	{
	...
	bool IrisD3DResourceManager::InitializeDirectX10(IDXGIAdapter1* pAdapter)
	{
