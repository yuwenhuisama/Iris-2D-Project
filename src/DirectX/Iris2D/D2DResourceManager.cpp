...
namespace Iris2D
{
	...
	{
		static auto pInstance = D2DResourceManager();
		return &pInstance;
	}

	bool D2DResourceManager::Initialize()
	{
		...
		auto pD3DDevice = D3DResourceManager::Instance()->GetD3D11Device();
		// 创建 IDXGIDevice
	...
	*/
	...
	bool D2DResourceManager::CreateTexture(unsigned int nWidth, unsigned int nHeight, ID3D11Resource*& pTexture) {
		// CreateTexture
		...
		auto hResult = D3DResourceManager::Instance()->GetD3D11Device()->CreateTexture2D(&texDesc, nullptr, &pTmpTexture);
		if (FAILED(hResult)) {
	...
	bool D2DResourceManager::MakeSharedResource(ID3D11Resource* pTexture, HANDLE& hResourceShareHandle, IDXGIKeyedMutex*& pDX11Mutex) {
		pDX11Mutex = nullptr;
	...
	bool D2DResourceManager::CreateDxgiRenderTarget(HANDLE hResourceShareHandle, ID2D1RenderTarget*& pDxgiRenderTarget, IDXGIKeyedMutex*& pDX10Mutex) {
		// CreateRenderTarget
		IDXGISurface1* pDxgiSurface = nullptr;
		auto hResult = D3DResourceManager::Instance()->GetD3D10Device()->OpenSharedResource(hResourceShareHandle, __uuidof(IDXGISurface1), reinterpret_cast<LPVOID*>(&pDxgiSurface));
		if (FAILED(hResult)) {
			goto failed_release;
		}
	...
	bool D2DResourceManager::LoadBitmapFromFile(
		...
		hResult = D3DResourceManager::Instance()->GetD3D11Device()->CreateShaderResourceView(pTexture, &srvDesc, &pResourceView);
		if (FAILED(hResult)) {
			SafeCOMRelease(pTexture);
	...
	bool D2DResourceManager::CreateBlankTexture(
		unsigned int nWidth,
	...
	ID2D1Factory1 * D2DResourceManager::GetD2DFactory() const
	{
		return m_pD2DFactory;
	}

	ID2D1DeviceContext * D2DResourceManager::GetD2DDeviceContext() const
	{
		return m_pD2DDeviceContext;
	}

	IDWriteFactory3 * D2DResourceManager::GetDWriteFactory() const
	{
		return m_pDWriteFactory;
	}

	bool D2DResourceManager::Release()
	{
		SafeCOMRelease(m_pD2DFactory);
		SafeCOMRelease(m_pD2DDevice);
		SafeCOMRelease(m_pD2DDeviceContext);
